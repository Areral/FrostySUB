...
CONFIG = {
    ...
    "THREADS": 10, "TCP_TIMEOUT": 2, "PIPELINE_TIMEOUT": 20,
    
    "SPEED_TEST_URL": "http://speed.cloudflare.com/__down?bytes=8000000",
    "MIN_SPEED_MBPS": 2.5,   # <-- СНИЖЕН ПОРОГ (Больше стабильных узлов)
    ...
    "GEO_API": "https://ipwho.is/", "USER_AGENT": "v2rayNG/1.8.5"
}
...
class SunnyBot:
    ...
    async def run(self):
        logger.info(f"🚀 V6.5 UX UPDATE (MSK: {msk_now().strftime('%H:%M:%S')})")
        nodes = await self.get_nodes()
        tcp_res = await asyncio.gather(*[self.tcp_check(n, asyncio.Semaphore(50)) for n in nodes])
